...
export async function POST(req: NextRequest) {
    try {
        ...
        try {
            pythonResponse = await fetch(`${PYTHON_SERVICE_URL}/unlock/pdf`, {
                method: "POST",
                body: forwardForm,
            });
        } catch (networkErr) {
            ...
            return NextResponse.json(
                {
                    error: "The unlock service is not running. Please start the Python microservice.",
                },
                { status: 503 }
            );
        }
        ...
        if (!pythonResponse.ok) {
            ...
            return NextResponse.json(
                { error: errorBody || `Unlock failed with status ${pythonResponse.status}` },
        ...
        console.error("Unlock PDF API route error:", error);
        return NextResponse.json(
