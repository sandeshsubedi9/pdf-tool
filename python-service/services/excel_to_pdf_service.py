...
def _find_libreoffice() -> str:
    ...
    if sys.platform == "win32":
        ...
        raise RuntimeError(
            ...
            "On Ubuntu/Debian run: sudo apt-get install -y libreoffice-calc"
        )
...
def _run_libreoffice(soffice: str, input_path: str, output_dir: str) -> None:
    ...
    logger.info(f"Running LibreOffice for Excel conversion: {' '.join(cmd)}")
...
async def convert_excel_to_pdf(file: UploadFile) -> tuple[bytes, str]:
    """
    Converts an Excel spreadsheet (.xls / .xlsx / .csv) to PDF using LibreOffice.
    """
    ...
    logger.info(f"Successfully converted Excel '{filename}' to PDF")
